...
impl Pipeline {
    /// Builds a fixed in-process pipeline without configuration file watchers.
    ///
    /// Embedders can create a pipeline for an explicit request profile while
    /// keeping Koharu's model residency and scheduling behavior intact.
    pub fn from_values(
        ...
        let runner = StageRunner::new(&config, translator, &device, resources.clone())?;
        Ok(Self {
